...
//   node tools/qa/capture-project-images.mjs <outDir> [mihsabUrl] [lcDeskFileUrl]
import { chromium } from 'playwright';
const [out = 'shots', mihsab, lcDesk] = process.argv.slice(2);
...
if (lcDesk) {
  await page.goto(lcDesk, { waitUntil: 'load' });
  await page.getByRole('button', { name: /Sample Word draft/i }).click();
  await page.waitForTimeout(2500);
  await page.screenshot({ path: `${out}/lc-amendment-desk.png` });
}
await browser.close();
